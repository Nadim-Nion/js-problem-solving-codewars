...
const sumAverage = (arr) => {
    const averages = arr.map(subArray => {
        const sum = subArray.reduce((total, currentValue) => total + currentValue, 0);
        return sum / subArray.length;
    });

    const total = averages.reduce((total, currentValue) => total + currentValue, 0);
